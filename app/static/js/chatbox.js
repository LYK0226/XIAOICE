...
function createMessage(text, isUser = false) {
    ...
    paragraph.innerHTML = renderMessageText(text);
    messageContent.appendChild(paragraph);

// Helper to safely render message text and replace :owl: placeholder with the owl image
function escapeHtml(str) {
    return str.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
}

function renderMessageText(str) {
    // Escape user text, then replace the :owl: token with the owl img (not escaped)
    const escaped = escapeHtml(str);
    return escaped.replace(/:owl:/g, '<img src="/static/upload/cute-owl-owl.gif" class="emoji-owl" alt="owl" />');
}
...
function populateEmojis(category) {
    ...
    emojis.forEach(emoji => {
        ...
        // Special owl marker renders as image in picker but inserts a placeholder into the input
        if (emoji === '__OWL_IMG__') {
            emojiSpan.innerHTML = '<img src="{{ url_for(\"static\", filename=\"upload/cute-owl-owl.gif\") }}" class="emoji-owl" alt="owl" />';
        } else {
            emojiSpan.textContent = emoji;
        }
        emojiSpan.addEventListener('click', () => {
            // Insert emoji or owl placeholder at cursor position
            const start = messageInput.selectionStart;
            const end = messageInput.selectionEnd;
            const text = messageInput.value;
            if (emoji === '__OWL_IMG__') {
                const placeholder = ':owl:';
                messageInput.value = text.substring(0, start) + placeholder + text.substring(end);
                messageInput.selectionStart = messageInput.selectionEnd = start + placeholder.length;
            } else {
